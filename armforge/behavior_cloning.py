...
class BehaviorCloning:
    ...
    def __init__(self, env, cfg: dict, teacher: nn.Module | None, device: str = "cpu"):
        self._env = env
        self._cfg = cfg
        self._device = device
        self._teacher = teacher
        self._num_steps_per_env = cfg["num_steps_per_env"]
        self._human_demo_dir = cfg.get("human_demo_dir")
        self._human_mix_ratio = float(cfg.get("human_mix_ratio", 0.3))

        # Mono rgb from downscaled episode camera
        rgb_shape = (3, env.image_height, env.image_width)
        ...
        self._human_demos = self._load_human_demos() if self._human_demo_dir else None
    ...
    def learn(self, num_learning_iterations: int, log_dir: str) -> None:
        ...
        for it in range(num_learning_iterations):
            ...
            for batch in generator:
                ...
                pred_pose = self._policy.predict_pose(batch["rgb_obs"])

                # Compute action prediction loss
                action_loss = F.mse_loss(pred_action, batch["actions"])

                # Compute pose estimation loss (position + orientation)
                pose_loss = self._compute_pose_loss(pred_pose, batch["object_poses"])
    ...
    def _load_human_demos(self) -> dict[str, torch.Tensor] | None:
        """Load LeRobot-style episode NPZ files into a demo tensor dict."""
        demo_dir = self._human_demo_dir
        if demo_dir is None or not os.path.isdir(demo_dir):
            return None
        rgb_list, pose_list, obj_list, act_list = [], [], [], []
        for name in sorted(os.listdir(demo_dir)):
            if not name.endswith(".npz"):
                continue
            data = np.load(os.path.join(demo_dir, name))
            rgb_list.append(torch.as_tensor(data["rgb"], device=self._device))
            pose_list.append(torch.as_tensor(data["ee_pose"], device=self._device))
            obj_list.append(torch.as_tensor(data["object_pose"], device=self._device))
            act_list.append(torch.as_tensor(data["action"], device=self._device))
        if not rgb_list:
            return None
        print(f"[ArmForge] Loaded {len(rgb_list)} human demo episodes from {demo_dir}")
        return {
            "rgb": torch.cat(rgb_list, dim=0),
            "ee_pose": torch.cat(pose_list, dim=0),
            "object_pose": torch.cat(obj_list, dim=0),
            "action": torch.cat(act_list, dim=0),
        }

    def _inject_human_demos(self) -> None:
        """Mix human teleop frames into the online buffer."""
        if self._human_demos is None or self._human_demos["action"].shape[0] == 0:
            return
        n = self._human_demos["action"].shape[0]
        n_sample = max(1, int(self._env.num_envs * self._human_mix_ratio))
        idx = torch.randint(0, n, (n_sample,), device=self._device)
        rgb = self._human_demos["rgb"][idx]
        ee = self._human_demos["ee_pose"][idx]
        obj = self._human_demos["object_pose"][idx]
        act = self._human_demos["action"][idx]
        # Broadcast single-env demos across a fake env batch of size 1 then tile.
        for i in range(n_sample):
            rgb_i = rgb[i : i + 1]
            if rgb_i.shape[1] > 3:
                rgb_i = rgb_i[:, :3]
            self._buffer.add(
                rgb_i.expand(self._env.num_envs, -1, -1, -1),
                ee[i : i + 1].expand(self._env.num_envs, -1),
                obj[i : i + 1].expand(self._env.num_envs, -1),
                act[i : i + 1].expand(self._env.num_envs, -1),
            )
    ...
    def _collect_with_rl_teacher(self) -> None:
        ...
        with torch.inference_mode():
            for _ in range(self._num_steps_per_env):
                ...
                rgb_obs = self._env.get_rgb_images(normalize=True)
...
class Policy(nn.Module):
    """Behavior cloning policy with a single-camera vision encoder."""

    def __init__(self, config: dict, action_dim: int):
        super().__init__()

        self.encoder = self._build_cnn(config["vision_encoder"])
    ...
    def get_features(self, rgb_obs: torch.Tensor) -> torch.Tensor:
        return self.encoder(rgb_obs[:, :3]).flatten(start_dim=1)

    def forward(self, rgb_obs: torch.Tensor, state_obs: torch.Tensor | None = None) -> torch.Tensor:
        features = self.get_features(rgb_obs)
    ...
    def predict_pose(self, rgb_obs: torch.Tensor) -> torch.Tensor:
        return self.pose_mlp(self.get_features(rgb_obs))
