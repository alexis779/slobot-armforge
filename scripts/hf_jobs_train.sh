...
#   FLAVOR=a10g-large ITERS=3400 N_ENVS=4096 bash scripts/hf_jobs_train.sh
#
# Defaults target ~1h train wall from hybrid HF timing at N_ENVS=4096
# (steady ~1.05 s/iter → ITERS≈3400). No on-job probe (cold start ~15 min).
# Job TIMEOUT=3h so setup + train + Hub upload fit (prior 2h run was SIGTERM'd).
set -euo pipefail
...
# Timing from hybrid job 6a769a60… steady ~1.05 s/iter @ 4096.
SEC_PER_ITER="${SEC_PER_ITER:-1.05}"
TRAIN_BUDGET_S="${TRAIN_BUDGET_S:-3600}"
SEC_PER_ITER_SOURCE="${SEC_PER_ITER_SOURCE:-hf_job_6a769a60_hybrid_n4096}"
...
"${HF_CMD[@]}" jobs run \
  --flavor "$FLAVOR" \
  --timeout "$TIMEOUT" \
  --name "${JOB_NAME:-armforge-hybrid-ppo}" \
...
if log.exists():
    ...
    print('No log dir to upload:', log)
PY
}
